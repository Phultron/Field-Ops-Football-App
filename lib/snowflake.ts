...
function connectAndQueryLongRunning(
  ...
  return new Promise((resolve, reject) => {
    conn.connect((err) => {
      ...
      runLongRunningOnConnection(conn as unknown as PollingConnection, query, longOpts, warehouse)
        .then((rows) => {
          conn.destroy(() => {})
...
export async function querySnowflake(query: string, options: QueryOptions = {}): Promise<Record<string, any>[]> {
  const { callersRights = false, warehouse } = options
  ...
  if (serviceToken) {
    if (callersRights) {
      ...
      return queryWithPool(getCallersPool(combinedToken, serviceToken), query, "spcs-caller", warehouse)
    }
    // Retry once on terminated connection (service token rotated mid-flight).
    try {
      return await queryWithPool(getOwnersPool(serviceToken), query, "spcs-owner", warehouse)
    } catch (err) {
      ...
      if (msg.includes("terminated connection")) {
        ...
        ownersPool?.drain()
        ownersPool = null
        return queryWithPool(getOwnersPool(serviceToken), query, "spcs-owner", warehouse)
      }
      throw err
    }
  }
  ...
  if (process.env.SNOWFLAKE_USER && process.env.SNOWFLAKE_PASSWORD) {
    return queryWithPool(getPasswordPool(), query, "password", warehouse)
  }

  // ~/.snowflake/connections.toml or config.toml: use the default connection (local dev)
  const tomlConn = readTomlDefaultConnection()
  if (tomlConn) {
    return queryWithPool(getTomlPool(tomlConn), query, "toml", warehouse)
  }
...
export async function querySnowflakeLongRunning(
  ...
  const { callersRights = false, warehouse, ...longRest } = options
  ...
  if (serviceToken) {
    ...
    return queryWithPoolLongRunning(getOwnersPool(serviceToken), query, "spcs-owner", longOpts, warehouse)
  }
  ...
  if (process.env.SNOWFLAKE_USER && process.env.SNOWFLAKE_PASSWORD) {
    return queryWithPoolLongRunning(getPasswordPool(), query, "password", longOpts, warehouse)
  }

  const tomlConn = readTomlDefaultConnection()
  if (tomlConn) {
    return queryWithPoolLongRunning(getTomlPool(tomlConn), query, "toml", longOpts, warehouse)
  }
 ...
 */
export async function queryWithToken(query: string, token: string, warehouse?: string): Promise<Record<string, any>[]> {
  return connectAndQuery({ ...baseConfig(), authenticator: "OAUTH", token }, query, "oauth-token", warehouse)
}
...
export async function queryWithTokenLongRunning(
  ...
  const { warehouse, ...longRest } = options
